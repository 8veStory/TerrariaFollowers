...
namespace gaomonmod1dot4.Items
{
    public class BlueDigivice : ModItem
    {
        ...
        {
            return player.whoAmI == Main.myPlayer && !terraguardians.PlayerMod.PlayerHasCompanion(player, 0, this.Mod.Name)  //Player doesn't have Gaomon in their companion list
            && 
        ...
        public override bool CanUseItem(Player player)
        {
            if (UseItem(player))
            {
                ...
                terraguardians.Companion tg = 
                terraguardians.WorldMod.SpawnCompanionNPC(player.Bottom, FollowersContainer.Glory, Mod.Name); //Spawns Gaomon as a companion npc, on the position of the player (Companion spawn orientation is based on the bottom position, so aim for the feet)
